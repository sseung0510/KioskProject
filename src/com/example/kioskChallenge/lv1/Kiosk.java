...
public class Kiosk {
    ...
    public void start() {
        ...
        while(true) {
            ...
            if(menuNum == 0) { // 종료
                break; // 상위 반복문 탈출
            } else if((menuNum == 4 || menuNum == 5) && cartMenu.isEmpty()) {
            ...
            while(true) {
                ...
                System.out.println('"' + menuItem.getAll() + '"' + "\n위 메뉴를 장바구니에 추가하시겠습니까?");
                System.out.println("1. 확인    |    2. 취소");
                int choiceNum = sc.nextInt();
                if(choiceNum == 1) {
                    cartMenu.add(menuItem.getAll());
                    cartPrice.add(menuItem.getPrice());
    ...
    private void orderMenu() {
        ...
        for(int i = 0; i < cartMenu.size(); i++) {
            System.out.println(cartMenu.get(i));
        }

        for(int i = 0; i < cartMenu.size(); i++) {
            paySum += cartPrice.get(i);
        }

        System.out.println("\n[ 총 ]");
        System.out.println("w " + paySum);

        System.out.println("\n1. 주문    |    2. 메뉴판");
        int choiceNum = inputNumException();
        if(choiceNum == 1) {
            System.out.println("주문이 완료되었습니다. 금액은 w" + paySum + "입니다.");
            cartMenu.clear();
        } else if(choiceNum == 2) {
            System.out.println();
        } else {
