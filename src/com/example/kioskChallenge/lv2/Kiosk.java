...
public class Kiosk {
    ...
    private void orderMenu() {
        ...
        System.out.println("\n[ 총 ]");
        System.out.println("w " + Math.round(totalPay * 1000.0) / 1000.0);

        System.out.println("\n1. 주문    |    2. 메뉴판    |    3. 메뉴 취소");
        int choiceNum = inputNumException("해당 번호만 선택해주세요: ");

        if (choiceNum == 1) {
            double pay = discountPay(Math.round(totalPay * 1000.0) / 1000.0);
            System.out.println("주문이 완료되었습니다. 금액은 w " + Math.floor(pay * 10.0) / 10.0 + "입니다.");
            cart.clearCart();
        } else if (choiceNum == 2) {
            System.out.println();
        } else if (choiceNum == 3) {
            int deleteNum = inputNumException("취소할 메뉴: ");
            cart.deleteMenu(deleteNum);
        } else {
            System.out.println("없는 선택지입니다.");
        }
    }
     ...
     * 할인 기능
     */
    public double discountPay(double totalPay) {
        ...
        int choiceDiscount = inputNumException("선택해주세요: ");

        if(choiceDiscount == 1) {
            totalDiscount = Coupon.NATIONALHERO.applyDiscount(totalPay);
        } else if(choiceDiscount == 2) {
            totalDiscount = Coupon.SOLDIER.applyDiscount(totalPay);
        } else if(choiceDiscount == 3) {
            totalDiscount = Coupon.STUDENT.applyDiscount(totalPay);
        } else if(choiceDiscount == 4) {
            totalDiscount = Coupon.NORMAL.applyDiscount(totalPay);
        } else {
